...
class LocalDataSource(private val database: FinEvoDatabase) {
    ...
    // ============================================
    // USER (Offline-First Auth Cache)
    // ============================================

    /** Get cached user by ID as Flow */
    fun getUser(userId: String): Flow<User?> {
        return queries.selectUserById(userId).asFlow().mapToOneOrNull(Dispatchers.IO).map {
            it?.toDomainUser()
        }
    }
    ...
    suspend fun insertUser(user: User) =
        withContext(Dispatchers.IO) {
            ...
            queries.insertUser(
                ...
                tier = user.tier.name,
                is_malaysian = if (user.isMalaysian) 1L else 0L,
    ...
    suspend fun deleteUser(userId: String) =
        withContext(Dispatchers.IO) { queries.deleteUser(userId) }
    ...
    suspend fun getCategoryById(id: String): Category? =
        withContext(Dispatchers.IO) {
            queries.selectAllCategories(defaultUserId)
                .executeAsList()
                .find { it.id == id }
                ?.toDomainCategory()
        }
            ...
            queries.insertHabit(
                ...
                target_days =
                    if (habit.targetDays.isNotEmpty())
                        habit.targetDays.joinToString(",")
                    else null,
                target_count = habit.targetCount.toLong(),
                reminder_time = habit.reminderTime?.toString(),
                reminder_enabled = if (habit.reminderEnabled) 1L else 0L,
...
private fun Habits.toDomainHabit(): Habit =
    Habit(
        ...
        icon = icon,
        color = color,
        targetDays = target_days?.split(",")?.mapNotNull { it.trim().toIntOrNull() }
            ?: emptyList(),
        ...
        reminderEnabled = reminder_enabled == 1L,
